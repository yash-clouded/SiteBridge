...
function Kpi({
  ...
  /** Status KPIs take one of the system's status colours; others stay primary. */
  tone?: string;
}) {
  return (
    <div className="rounded-md border border-line bg-surface px-4 py-3">
      <div className="text-[12px] text-muted">{label}</div>
      <div className={`mt-1 text-[26px] font-semibold leading-7 tabular-nums ${tone ?? "text-ink"}`}>
        {value}
      </div>
      <div className="mt-1 text-[12px] leading-4 text-muted">{hint}</div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl space-y-5">
        <section className="flex items-center justify-between rounded-md border border-line bg-surface px-4 py-2.5">
        ...
        {projects.length === 0 ? (
          <section className="rounded-md border border-banner-warning-icon/30 bg-banner-warning-bg px-4 py-6 text-[13px] text-banner-warning-text">
          ...
          <section className="rounded-md border border-status-rejected/30 bg-status-rejected-bg px-4 py-3 text-[13px] text-status-rejected">
            {error}
          </section>
        ) : !rollup || loading ? (
          <section className="rounded-md border border-line bg-surface px-4 py-3 text-[13px] text-muted">
            Loading roll-up…
          </section>
        ) : (
          <>
            ...
            <section className="rounded-md border border-line bg-surface px-4 py-3">
              ...
              <div className="flex h-3 overflow-hidden rounded-sm border border-line bg-page">
                <div
                  className="bg-accent"
                  style={{
              ...
              <p className="mt-1.5 text-[12px] text-muted">
                An activity without approved evidence is <span className="text-muted">unreported</span>,
                never 0% complete.
              </p>
            </section>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
                  ...
                  <ul className="divide-y divide-line">
                    {rollup.series.map((point) => (
                      <li key={point.day} className="flex items-center gap-3 px-4 py-2 text-[13px]">
                        <span className="w-20 shrink-0 text-muted">{dayLabel(point.day)}</span>
              ...
              <section className="overflow-hidden rounded-md border border-line bg-surface">
                ...
                <ul className="divide-y divide-line text-[13px]">
                  {[
                    { label: "Awaiting decision", count: rollup.pending_review, tone: "text-status-pending" },
                    { label: "Needs manual mapping", count: rollup.needs_manual, tone: "text-status-corrected" },
                    { label: "Approved", count: rollup.approved, tone: "text-status-approved" },
                    { label: "Rejected", count: rollup.rejected, tone: "text-status-rejected" },
                  ].map((row) => (
                    <li key={row.label} className="flex items-center justify-between px-4 py-2">
                      <span className="text-muted">{row.label}</span>
                      <span className={`tabular-nums font-medium ${row.tone}`}>{row.count}</span>
                    </li>
                  ))}
                </ul>
            ...
            <section className="overflow-hidden rounded-md border border-line bg-surface">
              ...
              {buckets.length === 0 ? (
                <p className="px-4 py-3 text-[13px] text-muted">Nothing to break down yet.</p>
              ) : (
                      ...
                      {buckets.map((bucket) => (
                        <tr key={bucket.key || "—"} className="border-b border-line last:border-0">
                          <td className="max-w-[260px] truncate px-4 py-2 text-ink">
                            {bucket.key || <span className="text-muted">Not stated</span>}
                          </td>
                            ...
                            {Object.keys(bucket.by_status).length === 0 ? (
                              ...
                              <span className="flex flex-wrap gap-1">
                                {Object.entries(bucket.by_status).map(([label, count]) => (
                                  <span
                                    key={label}
                                    className="rounded border border-status-neutral/30 bg-status-neutral-bg px-1.5 py-0.5 text-[11px] text-status-neutral"
                                  >
                                    {label}: {count}
                                  </span>
                                ))}
